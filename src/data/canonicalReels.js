export const CANONICAL_REELS = [
  {
    id: 'ee000000-0000-0000-0000-000000000001',
    creatorHandle: '@cairo_chic',
    creatorName: 'كايرو شيك • Cairo Chic',
    avatar: '/images/reels/fashion_citrine_blazer_thumb.jpg',
    videoBg: '/images/reels/fashion_citrine_blazer.mp4',
    caption: 'تنسيق بليزر السيترين الأوفرسايز مع بنطلون جينز كلاسيك ونظارة شمسية 💛 فخامة الصيف وأناقة لا تقاوم! #بليزر #موضة_القاهرة',
    captionEn: 'Styling the oversized Citrine Yellow Blazer with denim and sleek shades 💛 Effortless luxury! #FashionReels #OOTD',
    music: 'ألحان إيقاعية هادية • صيف 2026',
    likes: 4820,
    comments: 3,
    saves: 1140,
    categoryId: 'fashion',
    qualityScore: 0.95,
    trendScore: 0.90,
    createdAt: '2026-06-01T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-blazer',
        sku: 'TLK-BLZ-09',
        title: 'بليزر أوفرسايز أصفر ليموني راقي',
        titleEn: 'Citrine Tailored Oversized Blazer',
        price: 2200,
        originalPrice: 2750,
        discount: '20% OFF',
        image: '/images/reels/fashion_citrine_blazer_thumb.jpg'
      },
      {
        id: 'p-fashion-jeans-1',
        sku: 'TLK-JNS-01',
        title: 'بنطلون جينز كلاسيك عالي الخصر',
        titleEn: 'Classic High-Waist Denim',
        price: 850,
        originalPrice: 1100,
        discount: '22% OFF',
        image: '/images/products/linen_abaya.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000002',
    creatorHandle: '@salma.styles',
    creatorName: 'سلمى ستايلز • Salma Styles',
    avatar: '/images/reels/fashion_oversized_shirt_thumb.jpg',
    videoBg: '/images/reels/fashion_oversized_shirt.mp4',
    caption: 'قميص كتان سماوي أوفرسايز خفيف جداً ومريح مع بنطلون تشينو بيج واسع 🩵 إطلالة كاجوال أنيقة لكل يوم!',
    captionEn: 'Sky blue linen oversized shirt paired with relaxed wide chinos 🩵 Summer perfection! #LinenStyle',
    music: 'نغمات كاجوال مصرية',
    likes: 3450,
    comments: 3,
    saves: 820,
    categoryId: 'fashion',
    qualityScore: 0.88,
    trendScore: 0.85,
    createdAt: '2026-06-02T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-oversized-shirt',
        sku: 'TLK-SHT-05',
        title: 'قميص كتان سماوي بقصة أوفرسايز',
        titleEn: 'Oversized Sky Blue Linen Shirt',
        price: 950,
        originalPrice: 1200,
        discount: '21% OFF',
        image: '/images/reels/fashion_oversized_shirt_thumb.jpg'
      },
      {
        id: 'p-fashion-chinos-1',
        sku: 'TLK-CHN-02',
        title: 'بنطلون تشينو واسع بيج',
        titleEn: 'Relaxed Beige Wide Chinos',
        price: 720,
        originalPrice: 900,
        discount: '20% OFF',
        image: '/images/products/embroidered_blouse.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000003',
    creatorHandle: '@zeina_ootd',
    creatorName: 'زينة أوفت • Zeina OOTD',
    avatar: '/images/reels/fashion_oneshoulder_top_thumb.jpg',
    videoBg: '/images/reels/fashion_oneshoulder_top.mp4',
    caption: 'توب بكتف واحد عاجي ناعم مع جينز كلاسيك عالي الخصر وحزام جلد 🤍 "Jeans and a cute top" هو الأساس دايماً!',
    captionEn: 'Asymmetric one-shoulder white bodysuit with high-waist denim 🤍 The classic "jeans and a cute top" equation! ✨',
    music: 'صوت تريند عالمي',
    likes: 2910,
    comments: 3,
    saves: 730,
    categoryId: 'fashion',
    qualityScore: 0.90,
    trendScore: 0.82,
    createdAt: '2026-06-03T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-oneshoulder-top',
        sku: 'TLK-TOP-06',
        title: 'توب بكتف واحد عاجي ناعم',
        titleEn: 'Asymmetric One-Shoulder White Top',
        price: 680,
        originalPrice: 850,
        discount: '20% OFF',
        image: '/images/reels/fashion_oneshoulder_top_thumb.jpg'
      },
      {
        id: 'p-fashion-jeans-2',
        sku: 'TLK-JNS-02',
        title: 'بنطلون جينز كلاسيك عالي الخصر',
        titleEn: 'Classic High-Waist Denim',
        price: 850,
        originalPrice: 1100,
        discount: '22% OFF',
        image: '/images/products/linen_abaya.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000004',
    creatorHandle: '@maya_accessories',
    creatorName: 'مايا إكسسوارات • Maya Accessories',
    avatar: '/images/reels/fashion_shoulder_bags_thumb.jpg',
    videoBg: '/images/reels/fashion_shoulder_bags.mp4',
    caption: 'سواتش كولكشن شنط الكتف الجلدية الكلاسيك بـ 5 ألوان تخطف العين (عاجي، بني، بوردو، رمادي، جملي) بإبزيم ذهبي فاخر! 👜✨',
    captionEn: 'All 5 iconic colorways of the structured leather shoulder bag with polished gold hardware! 👜✨ #Bags',
    music: 'إيقاع إكسسوارات راقي',
    likes: 5120,
    comments: 3,
    saves: 980,
    categoryId: 'accessories',
    qualityScore: 0.92,
    trendScore: 0.89,
    createdAt: '2026-06-04T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-shoulder-bags',
        sku: 'KHC-BAG-01',
        title: 'حقيبة كتف جلدية كلاسيك بإبزيم ذهبي',
        titleEn: 'Classic Structured Leather Bag',
        price: 1850,
        originalPrice: 2300,
        discount: '20% OFF',
        image: '/images/reels/fashion_shoulder_bags_thumb.jpg'
      },
      {
        id: 'p-fashion-silk-scarf',
        sku: 'TLK-SCF-01',
        title: 'سكارف حرير منقوش فاخر',
        titleEn: 'Luxury Printed Silk Scarf',
        price: 450,
        originalPrice: 600,
        discount: '25% OFF',
        image: '/images/products/silk_scarf.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000005',
    creatorHandle: '@layla_fashion',
    creatorName: 'ليلى فاشن • Layla Fashion',
    avatar: '/images/reels/fashion_knit_sweater_thumb.jpg',
    videoBg: '/images/reels/fashion_knit_sweater.mp4',
    caption: 'سويتر تريكو صوف أوفرسايز بياقة عالية مع جيبة صوف صوفية كاروهات 🖤 دفء وأناقة شتوية راقية!',
    captionEn: 'Chunky oversized knit turtleneck sweater paired with wool argyle skirt 🖤 Winter warmth & elegance!',
    music: 'موسيقى شتوية دافئة',
    likes: 2140,
    comments: 2,
    saves: 560,
    categoryId: 'fashion',
    qualityScore: 0.85,
    trendScore: 0.78,
    createdAt: '2026-06-05T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-knit-sweater',
        sku: 'TLK-SWT-07',
        title: 'سويتر صوف تريكو أوفرسايز بياقة عالية',
        titleEn: 'Chunky Knit Oversized Turtleneck',
        price: 1650,
        originalPrice: 2100,
        discount: '21% OFF',
        image: '/images/reels/fashion_knit_sweater_thumb.jpg'
      },
      {
        id: 'p-fashion-wool-skirt',
        sku: 'TLK-SKT-03',
        title: 'تنورة صوف بكسرات كاروهات',
        titleEn: 'Pleated Wool Argyle Skirt',
        price: 920,
        originalPrice: 1150,
        discount: '20% OFF',
        image: '/images/products/embroidered_blouse.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000006',
    creatorHandle: '@omar_looks',
    creatorName: 'عمر لوكس • Omar Looks',
    avatar: '/images/reels/fashion_suede_jacket_thumb.jpg',
    videoBg: '/images/reels/fashion_suede_jacket.mp4',
    caption: 'ستايل كلاسيكي رجالي راقي: جاكيت شمواه بني فاخر مع بنطلون زيتي وحذاء سويد مريح 🤎 قمة الفخامة الهادية!',
    captionEn: 'Refined menswear styling: Tobacco brown suede Harrington jacket with olive pleated trousers 🤎 #MenStyle',
    music: 'جاز مصري حديث',
    likes: 3120,
    comments: 2,
    saves: 610,
    categoryId: 'fashion',
    qualityScore: 0.89,
    trendScore: 0.84,
    createdAt: '2026-06-06T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-suede-jacket',
        sku: 'TLK-JCK-08',
        title: 'جاكيت شمواه كلاسيكي بني بسحاب',
        titleEn: 'Classic Suede Harrington Jacket',
        price: 2600,
        originalPrice: 3200,
        discount: '19% OFF',
        image: '/images/reels/fashion_suede_jacket_thumb.jpg'
      },
      {
        id: 'p-fashion-olive-pants',
        sku: 'TLK-PNT-04',
        title: 'بنطلون كلاسيك زيتي واسع',
        titleEn: 'Pleated Olive Trousers',
        price: 980,
        originalPrice: 1250,
        discount: '21% OFF',
        image: '/images/products/linen_trousers.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000007',
    creatorHandle: '@karim.editorial',
    creatorName: 'كريم إيديتوريال • Karim Editorial',
    avatar: '/images/reels/fashion_vintage_watch_thumb.jpg',
    videoBg: '/images/reels/fashion_vintage_watch.mp4',
    caption: 'تفاصيل الساعة البرميلية الكلاسيكية بطلاء الذهب الوردي وأرقام رومانية مع سويتر صوف عاجي أنيق ⌚✨ تحفة معصم!',
    captionEn: 'Vintage tonneau rose gold watch with roman dial & brown leather strap ⌚✨ Complete quiet luxury!',
    music: 'عزف بيانو كلاسيكي',
    likes: 5420,
    comments: 2,
    saves: 1240,
    categoryId: 'accessories',
    qualityScore: 0.94,
    trendScore: 0.91,
    createdAt: '2026-06-07T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-vintage-watch',
        sku: 'TBA-WTC-01',
        title: 'ساعة يد كلاسيكية برميليّة بعقارب رومانية',
        titleEn: 'Vintage Tonneau Rose Gold Watch',
        price: 3400,
        originalPrice: 4200,
        discount: '19% OFF',
        image: '/images/reels/fashion_vintage_watch_thumb.jpg'
      },
      {
        id: 'p-fashion-leather-bracelet',
        sku: 'TBA-BRC-02',
        title: 'سوار معصم جلد مضفر راقي',
        titleEn: 'Braided Leather Cuff Bracelet',
        price: 520,
        originalPrice: 650,
        discount: '20% OFF',
        image: '/images/brands/talieska_logo.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000008',
    creatorHandle: '@farida.atelier',
    creatorName: 'فريدة أتيليه • Farida Atelier',
    avatar: '/images/reels/fashion_woven_bag_thumb.jpg',
    videoBg: '/images/reels/fashion_woven_bag.mp4',
    caption: 'حقيبة الجلد المنسوجة يدوياً بحزام مضفر مع بنطلون كتان جملي واسع وميولز بيضاء 🤎 فخامة الحرف اليدوية المصرية!',
    captionEn: 'Handcrafted woven leather bag with braided handle styled with camel linen trousers & white mules! 🤎',
    music: 'أنغام ريترو هادية',
    likes: 3210,
    comments: 2,
    saves: 590,
    categoryId: 'accessories',
    qualityScore: 0.88,
    trendScore: 0.83,
    createdAt: '2026-06-08T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-woven-bag',
        sku: 'KHC-BAG-02',
        title: 'حقيبة جلد منسوجة يدوياً بمقبض مضفر',
        titleEn: 'Handcrafted Woven Leather Bag',
        price: 1950,
        originalPrice: 2500,
        discount: '22% OFF',
        image: '/images/reels/fashion_woven_bag_thumb.jpg'
      },
      {
        id: 'p-fashion-linen-pants-1',
        sku: 'TLK-LNN-01',
        title: 'بنطلون كتان جملي واسع',
        titleEn: 'Wide Camel Linen Trousers',
        price: 880,
        originalPrice: 1100,
        discount: '20% OFF',
        image: '/images/products/linen_abaya.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000009',
    creatorHandle: '@huda_leather',
    creatorName: 'هدى ليذر • Huda Leather',
    avatar: '/images/reels/fashion_barrel_bag_thumb.jpg',
    videoBg: '/images/reels/fashion_barrel_bag.mp4',
    caption: 'حقائب البولينج الأسطوانية الجلد الطبيعي في 4 ألوان فخمة (بني كروكو، كونياك سويد، نبيذي، أسود) 👜 سعة مذهلة وخياطة دقيقة!',
    captionEn: 'Luxury barrel bowling leather handbags in 4 rich finishes (croc dark brown, suede tan, deep wine, black) 👜',
    music: 'إيقاع استوديو القاهرة',
    likes: 2970,
    comments: 2,
    saves: 510,
    categoryId: 'accessories',
    qualityScore: 0.87,
    trendScore: 0.81,
    createdAt: '2026-06-09T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-barrel-bag',
        sku: 'KHC-BAG-03',
        title: 'حقيبة بولينج أسطوانية كلاسيك',
        titleEn: 'Barrel Bowling Handbag',
        price: 2100,
        originalPrice: 2600,
        discount: '19% OFF',
        image: '/images/reels/fashion_barrel_bag_thumb.jpg'
      },
      {
        id: 'p-fashion-leather-wallet',
        sku: 'KHC-WLT-01',
        title: 'محفظة جلد طبيعي مدمجة',
        titleEn: 'Compact Leather Cardholder',
        price: 490,
        originalPrice: 650,
        discount: '25% OFF',
        image: '/images/products/copper_lantern.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000010',
    creatorHandle: '@youssef_cairo',
    creatorName: 'يوسف ستايل • Youssef Cairo',
    avatar: '/images/reels/fashion_cuban_shirt_thumb.jpg',
    videoBg: '/images/reels/fashion_cuban_shirt.mp4',
    caption: 'قميص كوبي بنقشة ريترو مع بنطلون كتان أسود ونظارة كلاسيك 🕶️ إطلالة شبابية رايقة للصيف!',
    captionEn: 'Retro print Cuban collar shirt styled with black linen trousers 🕶️ Smooth summer aesthetic! #MenSummer',
    music: 'فانك مصري هادي',
    likes: 2840,
    comments: 2,
    saves: 430,
    categoryId: 'fashion',
    qualityScore: 0.86,
    trendScore: 0.79,
    createdAt: '2026-06-10T12:00:00.000Z',
    products: [
      {
        id: 'p-fashion-cuban-shirt',
        sku: 'TLK-SHT-06',
        title: 'قميص كوبي بنقشة ريترو',
        titleEn: 'Retro Print Cuban Collar Shirt',
        price: 850,
        originalPrice: 1100,
        discount: '22% OFF',
        image: '/images/reels/fashion_cuban_shirt_thumb.jpg'
      },
      {
        id: 'p-fashion-black-trousers',
        sku: 'TLK-TRS-09',
        title: 'بنطلون كتان أسود صيفي',
        titleEn: 'Black Linen Summer Trousers',
        price: 790,
        originalPrice: 990,
        discount: '20% OFF',
        image: '/images/products/linen_trousers.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000011',
    creatorHandle: '@beauty.by.nada',
    creatorName: 'ندى بيوتي • Nada Beauty',
    avatar: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=150&q=80',
    videoBg: '/images/reels/sheglam_mascara.mp4',
    caption: 'ريفيو جديد لماسكارا شيجلام! رهيبة بتطول وتكثف الرموش، ومعاها مزيل خاص بيها بيشيلها في ثواني! ✨👀 #Sheglam #ماسكارا #تجميل',
    captionEn: 'Testing the new SHEGLAM Ultra Lash Lift Mascara & Easy Lash Removal! Amazing results! ✨👀 #Sheglam #Makeup',
    music: 'تريند بيوتي • تيك توك',
    likes: 4520,
    comments: 2,
    saves: 890,
    categoryId: 'beauty',
    qualityScore: 0.93,
    trendScore: 0.94,
    createdAt: '2026-06-11T12:00:00.000Z',
    products: [
      {
        id: 'p-sheglam-1',
        sku: 'SHG-MASC-01',
        title: 'ماسكارا شيجلام لتكثيف الرموش',
        titleEn: 'SHEGLAM Ultra Lash Lift Mascara',
        price: 350,
        originalPrice: 400,
        discount: '12% OFF',
        image: '/images/reels/sheglam_mascara_thumb.jpg'
      },
      {
        id: 'p-sheglam-1-remover',
        sku: 'SHG-REM-01',
        title: 'مزيل مكياج ومسكارا سريع',
        titleEn: 'SHEGLAM Easy Lash Removal',
        price: 150,
        originalPrice: 200,
        discount: '25% OFF',
        image: '/images/reels/sheglam_mascara_thumb.jpg'
      }
    ]
  },
  {
    id: 'ee000000-0000-0000-0000-000000000012',
    creatorHandle: '@makeup.with.sara',
    creatorName: 'سارة ميكأب • Sara Makeup',
    avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=150&q=80',
    videoBg: '/images/reels/sheglam_liptint.mp4',
    caption: 'سواتش لدرجات مرطب ومورد الشفاه والخدود من شيجلام 🍒 درجات تجنن (Cherry Bark, Plum Sauce, Bare Blush) ثبات وترطيب خيالي! 💋 #مكياج #شيجلام',
    captionEn: 'SHEGLAM Jelly Lip Tint & Blusher swatches 🍒 Gorgeous shades (Cherry Bark, Plum Sauce) and amazing hydration! 💋 #LipTint #Sheglam',
    music: 'موسيقى ريلز هادية',
    likes: 3870,
    comments: 2,
    saves: 650,
    categoryId: 'beauty',
    qualityScore: 0.91,
    trendScore: 0.92,
    createdAt: '2026-06-12T12:00:00.000Z',
    products: [
      {
        id: 'p-sheglam-2',
        sku: 'SHG-LIP-02',
        title: 'ملمع ومورد شفاه وبلاشر شيجلام',
        titleEn: 'SHEGLAM Jelly Lip Tint & Blusher',
        price: 280,
        originalPrice: 350,
        discount: '20% OFF',
        image: '/images/reels/sheglam_liptint_thumb.jpg'
      },
      {
        id: 'p-sheglam-3-brush',
        sku: 'SHG-BRS-01',
        title: 'فرشاة دمج البلاشر',
        titleEn: 'SHEGLAM Blending Brush',
        price: 120,
        originalPrice: 180,
        discount: '33% OFF',
        image: '/images/reels/sheglam_liptint_thumb.jpg'
      }
    ]
  }
];
