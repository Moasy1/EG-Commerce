...
export default function ProfileCloset() {
  ...
  return (
    <div className="w-full flex-1 max-w-4xl mx-auto px-4 md:px-6 py-4 pb-28 md:pb-12 text-on-surface text-right">
      {/* Profile Header Card matching 10_profile_closet_ar */}
      <div className="rounded-2xl bg-surface-container-lowest border border-surface-container-high p-5 mb-6 shadow-sm flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 text-center sm:text-right">
        <div className="flex flex-col sm:flex-row items-center gap-3.5">
          ...
          />
          <div>
            <h1 className="font-serif text-lg md:text-xl font-bold text-on-surface">مريم القاضي</h1>
      ...
      <div className="rounded-xl bg-surface-container-lowest border border-surface-container-high p-3.5 mb-6 shadow-xs">
        <h3 className="text-xs font-bold text-on-surface mb-2.5">تبديل الأدوار في المنصة</h3>
        <div className="grid grid-cols-3 gap-2">
          {[
            { id: 'buyer', label: '👤 مشتري', desc: 'تصفح الريلز والسوق' },
            { id: 'creator', label: '🎬 صانع محتوى', desc: 'الحملات والعمولات' },
            { id: 'merchant', label: '🏪 تاجر', desc: 'إدارة المنتجات والحملات' }
          ].map(r => (
            <button
              key={r.id}
              onClick={() => {
        ...
        <div className="space-y-3">
          <div className="rounded-xl bg-surface-container-lowest border border-surface-container-high p-4 shadow-xs">
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-surface-container-high">
              <div>
                <span className="text-xs font-bold text-on-surface">طلب رقم #EG-984201</span>
                <span className="text-[10px] text-on-surface-variant block mt-0.5">11 سبتمبر 2026</span>
              </div>
              <button
            ...
            <div className="flex items-center justify-between text-xs text-on-surface-variant">
              <span>فستان كتان صيفي + حقيبة كانفاس يدوية</span>
              <span className="font-serif font-bold text-on-surface">2,370 ج.م</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {INITIAL_PRODUCTS.slice(0, 2).map((item) => (
            <div key={item.id} className="rounded-xl bg-surface-container-lowest border border-surface-container-high p-2.5 shadow-xs">
              ...
              <span className="font-serif text-xs font-bold text-secondary mt-1 block">{item.price.toLocaleString()} ج.م</span>
            </div>
          ))}
        </div>
      )}
      ...
      {profileTab === 'settings' && (
        <div className="space-y-3 text-xs">
          <div className="rounded-xl bg-surface-container-lowest border border-surface-container-high p-3.5 shadow-xs">
            <h4 className="font-bold text-on-surface mb-1">العناوين المحفوظة</h4>
            <p className="text-on-surface-variant text-[11px]">القاهرة، مصر الجديدة، شارع الثورة عمارة 14 (الافتراضي)</p>
          </div>
          <div className="rounded-xl bg-surface-container-lowest border border-surface-container-high p-3.5 shadow-xs">
            <h4 className="font-bold text-on-surface mb-1">تطبيق الهاتف وتثبيت PWA</h4>
            <p className="text-on-surface-variant text-[11px] leading-relaxed mb-2.5">
              يمكنك تثبيت EG Fashion كتطبيق هاتف مستقل عبر الضغط على "إضافة للشاشة الرئيسية" في متصفحك.
            </p>
            <button
